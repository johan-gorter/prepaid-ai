...
There is no local pre-push hook — run the relevant checks yourself before pushing (`npm -s run build`, `npm -s run test:ct`, and the E2E suite with the emulator services running) when you want local confidence, but CI is the gate that blocks deploys.
...
- All app-managed client-side persistence lives in a single IndexedDB database (`payasyougo`, store `kv`) accessed through `src/composables/useIdbStorage.ts` (`idbGet` / `idbSet` / `idbDelete` / `idbClearAll`). **Do not use `window.localStorage` for new state** — pick a string key, store the value (Blob or JSON-serialisable object) in the unified store, and clear it on sign-out. The store currently holds: impression source / mask / draft Blobs, the chat conversation draft, the resolved Storage download URL cache, the user's color-scheme preference, the last-page redirect hint, the last-activity throttle timestamp, and the pending credit-purchase intent.
- Sign-out (`useAuth.signOut`) calls `idbClearAll()` to drop the whole database so a different user on the same device cannot recover the previous session's drafts, masks, cached URLs, or pending purchases.
- The IndexedDB database is shared across all tabs of the origin. Each connection opened in `useIdbStorage.ts` registers `onversionchange` and closes itself when another tab requests a version change, so the sign-out `deleteDatabase` is not blocked by a second open tab (which would otherwise defer the wipe and leave the previous user's data recoverable). `open()` also rejects on `onblocked` and never caches a rejected promise, and the cold-start "resume last page" read uses a time-bounded `idbGetFast` — so a stalled or blocked open can never strand the app on a spinner. If multi-tab delete-blocking ever proves troublesome anyway, the agreed fallback is to *clear* the store contents in a normal readwrite transaction (which never blocks) rather than `deleteDatabase` the whole database.
- The client persists resolved Firebase Storage download URLs in IndexedDB keyed by Storage path (see `useStorageUrl.ts`), and reuses them before calling `getDownloadURL()` again. This reduces redundant URL-resolution requests and makes image rendering after refresh more likely to stay offline-capable once an image has been seen online.
- The credit-purchase flow (`/buy-credits`) writes the chosen amount + follow-up route into IndexedDB via `usePendingPurchase.ts` before redirecting to login or to Stripe. After payment succeeds the page resumes from that intent and navigates back to the original action page so the user can press Send / Generate again.
...
- **Before changing anything in the renovation funnel** (landing card, photo/mask/choose/paint wizard, buy-credits, login timing, result page, sharing), read [docs/viral-flow.md](docs/viral-flow.md) — it records deliberate product decisions and invariants that must not be "fixed" in passing.
- **Before changing AI prompts or region markings** (paint / remove / free-prompt / apply-material flows in `functions/src/ai.ts` and `src/components/MaskingCanvas.vue`), read [docs/nano-banana-prompting.md](docs/nano-banana-prompting.md) — it records proven and failed approaches so they aren't re-discovered. Iterate with the local harness `functions/ai-lab` (one Vertex call per run, results + inputs saved per run, quantitative checks in `functions/ai-lab/tools/`) instead of deploying to test.
- **To reuse the photo-input flow** (take photo / upload+crop / paste+crop), read [docs/photo-input.md](docs/photo-input.md) — the camera/crop mechanics are reusable components (`CameraCapture.vue`, `ImageCropper.vue`) hosted per-feature so each keeps its own copy and funnel; includes a recipe for new use cases and the materials-registry reuse pattern.
