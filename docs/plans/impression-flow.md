...
have titles. The first AI result is auto-starred as the "after". A timeline view
shows all impressions chronologically. The home page displays each renovation as
...
| `/renovation/:id`     | `renovation-detail` | `RenovationDetailPage.vue` | **Rewrite** — becomes Timeline page                           |
...
- Each card is a single square thumbnail showing a **diagonal before/after
  composite**.
- Tapping a card navigates to `/renovation/:id` (timeline).
...
- **Step 0 (Capture):** Title input removed. Only photo selection remains.
  `canGoNext` for step 0 changes to just `!!loadedImage.value`.
- **Steps 1-2 (Mask, Prompt):** Unchanged.
- **Step 3 (Processing):** Unchanged (spinner while uploading).
- **Step 4 (Result — NEW):** After the impression is created and the page would
  ...
  - **Three buttons** in a bottom bar:
    - **Timeline** — navigates to `/renovation/:id`
    - **Trash** — deletes the just-created impression, resets to step 1
...
### Page 3: Timeline Page (`/renovation/:id`) — rewrite of RenovationDetailPage

**Purpose:** Scrollable timeline of all impressions for a renovation.

**Layout:**

- **Header:** Back button (goes to `/`), no title text needed (or minimal
  "Timeline" label).
- **Scrollable list** (full height minus header and bottom bar):
  ...
  - **Each impression item** shows:
    ...
    - Tapping the image (not the star) navigates to
      ...
      from this result.
  - List **auto-scrolls to the bottom** on initial load so the most recent
...
1. **Load source image** — resolve the appropriate Storage path to a URL, load
   it as an `HTMLImageElement`.
2. **Step 1: Mark Area** — same canvas-based mask drawing as NewRenovationPage
   step 1. The source image is already square-fitted (all stored images are
   1000x1000).
3. **Step 2: Describe Change** — same prompt textarea.
4. **Step 3: Processing** — upload composite, create impression document.
5. **Step 4: Result** — same 3-button bar as NewRenovationPage step 4:
   - **Timeline** — navigate to `/renovation/:id`
...
do not change the star — the user manually stars from the timeline.
...
- **New: `deleteRenovation(renovationId)`** — deletes the renovation document,
  ...
  (originals, composites, results). Used by the trash button on the timeline.

### `useImpressions.ts`

- Change sort order from `desc` to **`asc`** (ascending by `createdAt`) to
  match the timeline's display order.
...
Used in three places: NewRenovationPage (step 4), NewImpressionPage (step 4),
and Timeline page. Extract as a component or keep inline (it's only 3 buttons).

```
┌─────────────┬─────────────┬─────────────┐
│  Timeline   │    Trash    │ Next Change  │
└─────────────┴─────────────┴─────────────┘
```

**Styling:** Fixed to bottom, equal-width buttons, same `#0f3460` / `#1a1a2e`
color scheme as the rest of the app. Timeline button highlighted when on the
timeline page.

**Behavior per context:**

| Context                    | Timeline                | Trash                                  | Next Change                                  |
| -------------------------- | ----------------------- | -------------------------------------- | -------------------------------------------- |
| NewRenovationPage (step 4) | Go to `/renovation/:id` | Delete impression, reset to step 1     | Go to `/renovation/:id/new?source=<id>`      |
| NewImpressionPage (step 4) | Go to `/renovation/:id` | Delete impression, reset to step 1     | Go to `/renovation/:id/new?source=<id>`      |
| Timeline page              | No-op (active)          | Delete renovation (confirm), go to `/` | Go to `/renovation/:id/new?source=<afterId>` |
...
| `src/views/RenovationDetailPage.vue` | Full rewrite as Timeline page                                                              |
...
3. **NewRenovationPage.vue** — Remove title, add step 4 with 3-button bar and
   auto-star.
4. **NewImpressionPage.vue** — New file, extract shared mask/prompt logic from
   NewRenovationPage.
5. **RenovationDetailPage.vue** — Full rewrite as Timeline page.
